...
| FR-T-08 | Hồ sơ giảng viên hiện ảnh đại diện (`avatar_url`, cột `person.avatar_url`, nhập từ `archive.avatar` — chỉ nhận URL của chính kho `https://repository.ictu.edu.vn/…`, bỏ ảnh đặt chỗ do theme sinh ở domain khác) và lĩnh vực (`field`, cột `person.field`, từ `archive.knowsAbout`); `GET /api/persons/{id}` và `GET /api/persons?q=` (`PersonSearchRow`) trả thêm `avatar_url`. Giới tính, ngày sinh, điện thoại vẫn không đưa ra API/giao diện (dữ liệu cá nhân, xem README mục Quyền riêng tư) | ✅ |
| FR-T-09 | Xuất trích dẫn toàn bộ công bố của một giảng viên: `GET /api/persons/{id}/citations?style=apa\|ieee\|bibtex`, gồm mọi công trình đã liên kết còn sống (`DaNoiTuDong`/`DaXacNhan`), sắp theo năm giảm dần, mỗi mục cách nhau một dòng trống, tải về với tên tệp không dấu của giảng viên (`.bib`/`.txt`); khoá BibTeX trùng trong cùng tệp được thêm hậu tố a/b/c…; 404 nếu không có giảng viên. Tái dùng `cris.cite.apa/ieee/bibtex` đã dùng ở `/api/works/{id}/citation` | ✅ |
| FR-T-10 | Lọc công trình theo loại nơi công bố (`work.venue_kind` ∈ tạp chí/hội thảo × quốc tế/trong nước, hoặc chưa xác định): `GET /api/works?venue_kind=`; facet `venue_kinds` (nhãn tiếng Việt, chỉ đếm bài báo) ở `GET /api/works/facets`; `GET /api/works.csv` nhận thêm `venue_kind` | ✅ |
| FR-T-11 | Làm lộ đủ các loại công trình (lát cắt O1 — trên prod chỉ thấy bài báo vì đồ án/luận văn/luận án không ghi năm bị `year_issue DESC` đẩy xuống cuối): facet `doc_types` ở `GET /api/works/facets` (đủ 5 loại kể cả n=0, thứ tự cố định: Bài báo · Đồ án/Khoá luận · Luận văn ThS · Luận án TS · Học liệu số). `GET /api/works` đổi thứ tự mặc định thành `year_issue DESC NULLS LAST, cohort_num DESC NULLS LAST, id DESC` (số trích từ `work.cohort`, khoá mới nhất trước) và thêm `sort=recent\|title\|added` (422 nếu khác; `mode=semantic` bỏ qua, luôn theo điểm tương đồng); `GET /api/works.csv` nhận cùng `sort` | ✅ |
| FR-T-12 | `GET /api/stats` thêm `totals` (`works`, `persons`, `by_type`): đếm trên toàn bộ kho đang hoạt động, không giới hạn `years`, bỏ bản đã gộp — cùng điều kiện với `GET /api/works` (`v_work_current`) | ✅ |
| FR-T-13 | Danh bạ giảng viên công khai (lát cắt O1): `GET /api/persons/directory` — tìm theo tên (`q`, không dấu cũng khớp qua `person.name_norm`), lọc khoa (`unit`, mã/id/`none`), học hàm-học vị (`degree` ∈ `gs\|pgs\|ts\|ths\|other`), `has_works`; sắp theo số công trình (`sort=works`, mặc định) hoặc tên gọi tiếng Việt — từ cuối họ tên, ổn định (`sort=name`); phân trang; facet đơn vị/học vị đếm trên tập đã lọc theo `q`, không tự lọc theo chính facet đó. Chỉ `kind='lecturer'`, `active`; không email/điện thoại/ngày sinh. Khai báo trước `/persons/{pid}` trong router (`cris/api/app.py`) để không bị bắt nhầm là `pid` | ✅ |
...
| FR-K-10 | Minh chứng là tệp thật (lát cắt I2): loại nội dung nhận diện qua chữ ký byte, không tin phần mở rộng (`cris.declare.sniff_content_type` — pdf/png/jpeg/docx), tối đa 10 MB, băm SHA-256, lưu vào `CRIS_DATA_DIR/evidence/<id hồ sơ>/<sha256 rút gọn><đuôi>` (`add_evidence_file`, migration `0011_evidence_file.sql`); API `POST /api/declarations/{id}/evidence/file` (vai trò `rd_officer`, 413/415 khi quá khổ/sai loại), `GET /api/evidence/{id}/file` (`FileResponse`, cần đăng nhập khi auth bật, phạm vi đơn vị NFR-02 — 403 nếu khác đơn vị). Không hỗ trợ xoá minh chứng (minh chứng là bằng chứng, cố ý) | ✅ API xong, giao diện ⏳ ngoài bản dự thi |
| FR-K-11 | Bản báo cáo kỳ đóng băng, có phiên bản (lát cắt L1, điểm BA "P"): `build_report` chụp lại toàn bộ hồ sơ kê khai của kỳ (mọi trạng thái) cùng công trình/tác giả (tên hiển thị người đã liên kết hoặc tên thô)/minh chứng tại thời điểm gọi thành một dòng `period_report` mới (migration `0018_period_report.sql`) — `payload` đã ghi không đổi về sau dù dữ liệu gốc có đổi tiếp; `sha256` băm payload để kiểm lại. `POST /api/periods/{id}/reports` (`rd_officer`), `GET /api/periods/{id}/reports`, `GET /api/reports/{id}`, `GET /api/reports/{id}/export?format=csv\|xlsx` (`openpyxl`, sheet "Tổng hợp" + "Chi tiết") — cấp khoa chỉ thấy hồ sơ đơn vị mình. `finalize_period` tự sinh một bản báo cáo, trả thêm `report_id` | ✅ |
| FR-K-12 | Thông báo trong ứng dụng (lát cắt L2, điểm BA "H"): người dùng biết hồ sơ của mình đổi trạng thái. `cris/notify.py` (migration `0019_notification.sql`), gọi từ route sau khi nghiệp vụ thành công, không sửa `cris.declare`/`cris.link` — hồ sơ chuyển `ChoKhoaDuyet` → `faculty_head` cùng đơn vị; `KhoaDaDuyet`/`ChoPhongKiemTra` → mọi `rd_officer`; `DatYeuCau`/`DaChot`/trả về `Nhap`/`ChoBoSung`/`Rut` → người tạo hồ sơ (cộng `faculty_officer` cùng đơn vị khi trả về); kỳ báo cáo mở/đóng nộp/huỷ/chốt → `faculty_officer`/`faculty_head` mọi đơn vị (chốt kỳ trỏ tới bản báo cáo vừa sinh); liên kết tác giả được xác nhận (hàng đợi) → giảng viên có tài khoản. Không báo cho chính người thao tác; lỗi gửi thông báo không làm hỏng thao tác chính (bắt ngoại lệ, ghi log). `GET /api/notifications?unread=&page=`, `POST /api/notifications/{id}/read`, `POST /api/notifications/read-all` — chỉ chủ sở hữu đọc/đánh dấu được thông báo của mình | ✅ API xong, giao diện ⏳ ngoài bản dự thi |

### 3.6 Dòng lệnh (FR-C)

| Mã | Yêu cầu | Trạng thái |
|---|---|---|
| FR-C-01 | `python -m cris migrate\|seed\|sync\|people\|normalize\|link\|dedup\|quality\|serve\|units` — `sync --no-facets` tắt quét đơn vị theo dept; `people --assign-units` gán đơn vị giảng viên theo đa số; `units list\|rename\|alias` quản lý đơn vị | ✅ |
