...
|---|---|---|
| **Phòng KH-CN** | `rd_officer` | Toàn quyền trên toàn trường: mở/đóng/huỷ/chốt kỳ báo cáo, kiểm tra hồ sơ kê khai của mọi khoa, xử lý hàng đợi tác giả và nghi trùng, chỉnh tay dữ liệu công trình (có xuất xứ), quản trị tên/bí danh khoa (`/don-vi/`), bỏ qua cảnh báo bất thường, xem báo cáo và khoa bất kỳ. Giao diện gọi vai trò này là **"Chuyên viên KHCN"** ở các nút và tooltip (ví dụ tooltip **"Cần vai trò Chuyên viên KHCN"** khi bạn chưa đủ quyền) |
...
- Chip khoa trên một công trình lấy từ **hợp của hai nguồn**: khoa ghi ở kho nguồn (facet
...
**Cung cấp gì.**
- Dòng **"Khoa: CNTT, ĐTVT…"** ngay dưới tiêu đề — mỗi mã khoa là một chip bấm được, mở
  trang Tra cứu đã lọc sẵn theo đúng khoa đó; di chuột vào một chip để thấy tên đầy đủ.
  Không hiện dòng này nếu công trình chưa xác định được khoa nào.
...
- Khoa của giảng viên **không sửa được trên giao diện này**. Hệ thống tự gán (`unit_source
  = auto`) theo mã khoa xuất hiện nhiều nhất trong các công trình đã liên kết còn sống của
  người đó (hoà phiếu thì bỏ, không gán); Phòng KH-CN gán tay bằng dòng lệnh
  `python -m cris user set-unit --email <email> --unit <mã khoa>` khi cần sửa lại
...
**Tính năng này là gì.** Trang `/don-vi/` là danh mục **khoa thật** của trường — mã lấy
thẳng từ bộ lọc `dept` của kho nguồn (`repository.ictu.edu.vn`), tên và bí danh do Phòng
KH-CN đặt cho dễ hiểu. Đây không phải nơi tạo khoa mới hay xoá khoa — chỉ đặt tên và bí
danh cho các mã đã đồng bộ về.

**Cung cấp gì.**
- Bảng khoa: **Mã**, **Tên**, **Bí danh** (huy hiệu, có thể nhiều bí danh), **Công trình**
  (số công trình có khoa này), **Giảng viên** (số giảng viên được gán khoa này), cột
  **Hành động**.
- Nút **"Đổi tên"** trên mỗi dòng — mở hộp thoại đổi tên khoa.
- Nút **"Thêm bí danh"** trên mỗi dòng — mở hộp thoại thêm một bí danh mới cho khoa (ví dụ
...
**Cách sử dụng.**
1. Mở `/don-vi/` (mục **"Quản trị khoa"** trên thanh bên).
2. Bấm **"Đổi tên"** trên dòng khoa cần sửa, nhập **Tên mới \*** (bắt buộc, không được để
   trống), có thể ghi thêm **Lý do**, bấm **"Lưu tên"**.
3. Bấm **"Thêm bí danh"** để gán thêm một cách viết khác cho cùng một mã khoa, nhập
   **Bí danh \*** (bắt buộc), có thể ghi thêm **Lý do**, bấm **"Thêm bí danh"**.
4. Bảng khoa cập nhật ngay sau khi lưu.

**Ai được dùng.** Xem và sửa: chỉ **Phòng KH-CN** (`rd_officer`) — vai trò khác mở trang này
thấy thông báo **"Không có quyền truy cập"**.

**Lưu ý / giới hạn.**
- **Mã khoa lấy từ kho nguồn và không đổi được** ở đây — chỉ đổi được **tên** và thêm
...
| Mã | Tên hiển thị hiện tại | Ghi chú |
|---|---|---|
| `CNTT` | Khoa Công nghệ thông tin | Khoa hiện tại |
| `KT&CN` | Khoa Kỹ thuật và Công nghệ | Khoa hiện tại |
| `KT&QT` | Khoa Kinh tế và Quản trị | Khoa hiện tại; đổi tên năm 2011 từ Khoa Hệ thống thông tin kinh tế |
| `NT&TT` | Khoa Nghệ thuật và Truyền thông | Khoa hiện tại; tiền thân là Bộ môn Truyền thông đa phương tiện |
| *(chưa có mã ở nguồn)* | Khoa Khoa học liên ngành | Khoa hiện tại; tiền thân là Bộ môn Khoa học cơ bản |
| `ĐTVT` | Khoa Công nghệ điện tử và truyền thông | Tiền thân, nay thuộc KT&CN |
| `ĐTTT` | Khoa Điện tử truyền thông | Tiền thân, nay thuộc KT&CN |
| `TĐH` | Khoa Công nghệ tự động hoá | Tiền thân, nay thuộc KT&CN |
| `HTTTKT` (bí danh `HTTKT`) | Khoa Hệ thống thông tin kinh tế | Tiền thân, nay là KT&QT |
| `KHCB` | Khoa Khoa học cơ bản | Tiền thân, nay thuộc Khoa Khoa học liên ngành |
| `TTĐPT` | Khoa Truyền thông đa phương tiện | Tiền thân, nay thuộc NT&TT |

Các mã tiền thân **chưa được gộp** vào khoa hiện tại trên giao diện hay trong thống kê — mỗi
mã vẫn đứng riêng ở trang này, ở bộ lọc Tra cứu và ở chip công trình cho tới khi Phòng KH-CN
quyết định gộp (xem `docs/SRS.md` và kế hoạch lát cắt M).
...
| **Khoa** (mã hệ thống: `unit`) | Khoa thật của trường, mã lấy từ bộ lọc `dept` của kho nguồn (ví dụ `CNTT`, `KHCB`) — khác với **chức vụ** (Hiệu trưởng, Trưởng khoa…), vốn không phải một khoa. 10 mã đọc được ở nguồn ứng với 5 khoa hiện tại và một số khoa/bộ môn tiền thân còn xuất hiện ở bản ghi cũ — xem bảng mã ↔ tên chính thức ở mục 2.15. |
| **Chức vụ** (position) | Vai trò quản lý của một giảng viên (Hiệu trưởng, Phó Hiệu trưởng, Trưởng khoa…), hiển thị tách riêng khỏi khoa trên hồ sơ giảng viên. |
