...
**Files:**
- Create: `cris/quality.py`, `cris/cli.py`, `cris/__main__.py`, `cris/migrations/0004_views.sql`, `tests/test_quality.py`
...
`cris/migrations/0004_views.sql`:
...
Expected: `['0004_views.sql']` rồi tất cả test PASS (Task 1 đến 9).

- [ ] **Step 5: Commit**

```bash
git add cris/quality.py cris/cli.py cris/__main__.py cris/migrations/0004_views.sql cris/sync.py cris/source/repository.py cris/people.py tests/test_quality.py
...
- **Phủ spec §17.3:** 14 bảng có trong migration 0001; `v_work_unit`, `v_person_publications` ở 0001, `v_work_current`, `v_data_quality` ở 0004 (0002 sửa ràng buộc duy nhất của source_record ở Task 4, 0003 thu hẹp chỉ số DOI ở Task 8). `catalog` được tạo nhưng chưa có lệnh nạp danh mục vì Q-10 và Q-21 còn mở; ghi ở "Ngoài kế hoạch" ngầm qua kế hoạch giao diện.
