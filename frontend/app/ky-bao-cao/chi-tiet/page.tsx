...
import { Ban, FilePlus2, LockKeyhole, MoreHorizontal, Plus, RotateCcw, Search } from "lucide-react";
...
import {
  ...
  useOfficerAccess, usePeriodProgress, usePeriods, useSetDeclarationState, useStats, useWorks,
} from "@/lib/queries";
import type { DeclarationRow, DeclarationState, EvidenceKind } from "@/lib/types";
...
function DeclarationsTab({ periodId, periodState }: { periodId: number; periodState: string }) {
  ...
  const [stateAction, setStateAction] = useState<{ row: DeclarationRow; to: DeclarationState } | null>(null);
  ...
  async function transition(row: DeclarationRow, to: DeclarationState, reason?: string) {
    if (!canAct) return;
    try {
      await setState.mutateAsync({ id: row.id, input: { to_state: to, reason: reason?.trim() || null } });
      await invalidate();
      toast.success(to === "ChoBoSung" ? "Đã yêu cầu bổ sung hồ sơ." : to === "Nhap" ? "Đã trả hồ sơ về nháp." : "Đã rút hồ sơ.");
      setStateAction(null);
  ...
  return (
    <section aria-labelledby="declarations-title">
      ...
      {!declarations.data?.items.length ? <EmptyView title="Chưa có hồ sơ kê khai" description={unit === "all" ? "Kê khai công trình đầu tiên cho kỳ báo cáo này." : "Đơn vị đã chọn chưa có hồ sơ; hãy đổi bộ lọc hoặc kê khai công trình."} /> : (
        <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Hồ sơ</TableHead><TableHead className="min-w-72">Công trình</TableHead><TableHead>Loại</TableHead><TableHead>Đơn vị</TableHead><TableHead>Trạng thái</TableHead><TableHead className="text-right">Minh chứng</TableHead><TableHead>Cập nhật lần cuối</TableHead>{canAct && <TableHead><span className="sr-only">Hành động</span></TableHead>}</TableRow></TableHeader><TableBody>
          {declarations.data.items.map((row) => <TableRow key={row.id}>
            ...
            {canAct && <TableCell><DropdownMenu><DropdownMenuTrigger render={<Button type="button" variant="outline" size="sm" aria-label={`Hành động hồ sơ #${row.id}`} />}><MoreHorizontal />Hành động</DropdownMenuTrigger><DropdownMenuContent align="end">
              {row.state === "Nhap" && <DropdownMenuItem onClick={() => setStateAction({ row, to: "ChoBoSung" })}>Yêu cầu bổ sung</DropdownMenuItem>}
              {row.state === "ChoBoSung" && <DropdownMenuItem onClick={() => void transition(row, "Nhap")}><RotateCcw />Trả về nháp</DropdownMenuItem>}
              {row.state !== "Rut" && <DropdownMenuItem variant="destructive" onClick={() => setStateAction({ row, to: "Rut" })}><Ban />Rút</DropdownMenuItem>}
              <DropdownMenuItem onClick={() => setEvidenceRow(row)}><FilePlus2 />Thêm minh chứng</DropdownMenuItem>
      ...
      <Dialog open={stateAction !== null} onOpenChange={(open) => { if (!open) setStateAction(null); }}><DialogContent><form onSubmit={(event) => { event.preventDefault(); if (stateAction) void transition(stateAction.row, stateAction.to, String(new FormData(event.currentTarget).get("reason"))); }}><DialogHeader><DialogTitle>{stateAction?.to === "Rut" ? "Rút hồ sơ kê khai?" : "Yêu cầu bổ sung hồ sơ"}</DialogTitle><DialogDescription>{stateAction?.to === "Rut" ? "Hồ sơ sẽ chuyển sang trạng thái Đã rút. Xác nhận lý do trước khi tiếp tục." : "Nêu rõ nội dung đơn vị cần bổ sung để hồ sơ có thể được xử lý tiếp."}</DialogDescription></DialogHeader><div className="py-4"><label htmlFor="declaration-reason" className="mb-1.5 block font-medium">Lý do <span className="text-status-danger">*</span></label><Textarea id="declaration-reason" name="reason" required /></div><DialogFooter><Button type="button" variant="outline" onClick={() => setStateAction(null)}>Huỷ</Button><Button type="submit" variant={stateAction?.to === "Rut" ? "destructive" : "default"} disabled={!canAct || setState.isPending}>{stateAction?.to === "Rut" ? "Xác nhận rút" : "Yêu cầu bổ sung"}</Button></DialogFooter></form></DialogContent></Dialog>
