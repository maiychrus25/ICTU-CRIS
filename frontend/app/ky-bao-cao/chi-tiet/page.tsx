  ...
  return (
    <section aria-labelledby="declarations-title">
      <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div><h2 id="declarations-title" className="text-base font-semibold">Hồ sơ kê khai</h2><p className="text-xs text-muted-foreground">Công trình do các khoa kê khai trong kỳ báo cáo này.</p></div>
        <div className="flex flex-wrap items-end gap-2">
          <div><label className="mb-1 block text-xs font-medium">Khoa</label><Select value={unit} onValueChange={(value) => setUnit(String(value))}><SelectTrigger aria-label="Lọc theo khoa" className="w-56"><SelectValue>{(value) => value === "all" ? "Tất cả khoa" : stats.data?.by_unit.find((item) => item.unit_id === Number(value))?.code}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">Tất cả khoa</SelectItem>{stats.data?.by_unit.map((item) => <SelectItem key={item.unit_id} value={String(item.unit_id)}>{item.code} — {item.name}</SelectItem>)}</SelectContent></Select></div>
      ...
      {!declarations.data?.items.length ? <EmptyView title="Chưa có hồ sơ kê khai" description={unit === "all" ? "Kê khai công trình đầu tiên cho kỳ báo cáo này." : "Khoa đã chọn chưa có hồ sơ; hãy đổi bộ lọc hoặc kê khai công trình."} /> : (
        <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Hồ sơ</TableHead><TableHead className="min-w-72">Công trình</TableHead><TableHead>Loại</TableHead><TableHead>Khoa</TableHead><TableHead>Trạng thái</TableHead><TableHead className="text-right">Minh chứng</TableHead><TableHead>Cập nhật lần cuối</TableHead><TableHead><span className="sr-only">Hành động</span></TableHead></TableRow></TableHeader><TableBody>
      ...
      <Dialog open={declareOpen} onOpenChange={(open) => { setDeclareOpen(open); if (!open) setWorkId(null); }}><DialogContent className="sm:max-w-xl"><form onSubmit={(event) => void createDeclaration(event)}><DialogHeader><DialogTitle>Kê khai công trình</DialogTitle><DialogDescription>Chọn công trình và khoa chịu trách nhiệm kê khai trong kỳ này.</DialogDescription></DialogHeader><div className="space-y-4 py-4"><div><label className="mb-1.5 block font-medium">Công trình <span className="text-status-danger">*</span></label><WorkPicker key={String(declareOpen)} onValueChange={setWorkId} /></div><div><label className="mb-1.5 block font-medium">Khoa kê khai <span className="text-status-danger">*</span></label><Select value={unitId} onValueChange={(value) => setUnitId(String(value))}><SelectTrigger aria-label="Khoa kê khai" className="w-full"><SelectValue>{(value) => stats.data?.by_unit.find((item) => item.unit_id === Number(value))?.code ?? "Chọn khoa"}</SelectValue></SelectTrigger><SelectContent>{stats.data?.by_unit.map((item) => <SelectItem key={item.unit_id} value={String(item.unit_id)}>{item.code} — {item.name}</SelectItem>)}</SelectContent></Select></div><div><label htmlFor="declaration-note" className="mb-1.5 block font-medium">Ghi chú</label><Textarea id="declaration-note" name="note" /></div></div><DialogFooter><Button type="button" variant="outline" onClick={() => setDeclareOpen(false)}>Huỷ</Button><Button type="submit" disabled={!canAct || workId === null || !unitId || add.isPending}>Kê khai</Button></DialogFooter></form></DialogContent></Dialog>
