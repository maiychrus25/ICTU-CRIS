...
export default function PeriodsPage() {
  ...
  return (
    <>
      ...
      {query.isLoading ? <LoadingView /> : query.isError ? <ErrorView error={query.error} retry={() => query.refetch()} /> : !query.data?.length ? <EmptyView title="Chưa có kỳ báo cáo" description="Mở kỳ mới để các khoa bắt đầu kê khai công trình." action={<span title={canDecide ? undefined : officerRoleRequired}><Button type="button" variant="outline" onClick={() => setOpen(true)} disabled={!canDecide}>Mở kỳ mới</Button></span>} /> : <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Mã</TableHead><TableHead>Tên kỳ</TableHead><TableHead>Trạng thái</TableHead><TableHead>Hạn nộp</TableHead><TableHead>Ngày mở</TableHead><TableHead><span className="sr-only">Mở</span></TableHead></TableRow></TableHeader><TableBody>{query.data.map((period) => <TableRow key={period.id}><TableCell className="font-semibold tabular-nums">{period.code}</TableCell><TableCell><Link href={`/ky-bao-cao/chi-tiet/?id=${period.id}`} className="font-medium text-primary hover:underline">{period.name}</Link></TableCell><TableCell><StatusBadge value={period.state} /></TableCell><TableCell className="whitespace-nowrap tabular-nums">{formatDate(period.due_at)}</TableCell><TableCell className="whitespace-nowrap text-muted-foreground tabular-nums">{formatDate(period.opens_at)}</TableCell><TableCell><Button render={<Link href={`/ky-bao-cao/chi-tiet/?id=${period.id}`} aria-label={`Mở ${period.name}`} />} variant="ghost" size="icon-sm"><ExternalLink /></Button></TableCell></TableRow>)}</TableBody></Table></div>}
