...
export default function AboutPage() {
  ...
  if (query.isLoading) return <><PageHeader title="Về hệ thống" /><LoadingView /></>;
  if (query.isError) return <><PageHeader title="Về hệ thống" /><ErrorView error={query.error} retry={() => query.refetch()} /></>;
  ...
  return (
    <>
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_1.15fr]">
        ...
        <div className="space-y-7">
          <section><div className="mb-3 flex items-center gap-2"><Bot className="size-4 text-primary" /><h2 className="text-base font-semibold">AI trong hệ thống</h2><Badge variant="outline" className="ml-auto"><CheckCircle2 />Chỉ gợi ý</Badge></div><div className="rounded-lg border bg-card p-5"><div className="grid gap-x-8 sm:grid-cols-2"><Metric label="Nhà cung cấp" value={about.ai.provider} /><Metric label="Mô hình" value={about.ai.model} /><Metric label="Giấy phép" value={about.ai.licence} /><Metric label="Kích thước / số chiều" value={`${about.ai.size} · ${about.ai.dim}`} /><Metric label="Vector đã tạo" value={about.ai.embeddings.toLocaleString("vi-VN")} /><Metric label="Chủ đề / gợi ý" value={`${about.ai.topics} / ${about.ai.suggestions}`} /></div><a href={about.ai.repo} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">Xem mô hình và giấy phép <ExternalLink className="size-3.5" /></a></div></section>
