  ...
  return (
    <>
      ...
      <div className="mt-7 grid items-start gap-7 xl:grid-cols-2">
        <section className="min-w-0" aria-labelledby="unit-table-title"><h2 id="unit-table-title" className="mb-3 text-base font-semibold">Công trình theo đơn vị</h2>{stats.by_unit.length ? <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Mã</TableHead><TableHead className="whitespace-normal">Đơn vị</TableHead><TableHead className="text-right">Công trình</TableHead></TableRow></TableHeader><TableBody>{stats.by_unit.map((unit) => { const official = unitById.get(unit.unit_id); return <TableRow key={unit.unit_id}><TableCell className="font-medium">{official?.code ?? unit.code}</TableCell><TableCell className="whitespace-normal break-words">{official?.name ?? unit.name}</TableCell><TableCell className="text-right font-semibold tabular-nums">{unit.works.toLocaleString("vi-VN")}</TableCell></TableRow>; })}</TableBody></Table></div> : <EmptyView description="Chưa có dữ liệu đơn vị. Hãy chờ lần đồng bộ tiếp theo." />}</section>
        <section className="min-w-0" aria-labelledby="top-persons-title"><h2 id="top-persons-title" className="mb-3 text-base font-semibold">10 giảng viên có nhiều công trình nhất</h2>{stats.top_persons.length ? <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Giảng viên</TableHead><TableHead className="whitespace-normal">Đơn vị</TableHead><TableHead className="text-right">Công trình</TableHead></TableRow></TableHeader><TableBody>{stats.top_persons.map((person) => { const official = person.unit_code ? unitByCode.get(person.unit_code) : undefined; return <TableRow key={person.person_id}><TableCell><Link href={`/giang-vien/?id=${person.person_id}`} className="font-medium text-primary hover:underline">{person.display_name}</Link></TableCell><TableCell className="whitespace-normal break-words">{official ? `${official.code} — ${official.name}` : person.unit_code ?? "—"}</TableCell><TableCell className="text-right font-semibold tabular-nums">{person.works.toLocaleString("vi-VN")}</TableCell></TableRow>; })}</TableBody></Table></div> : <EmptyView description="Chưa có dữ liệu xếp hạng giảng viên. Hãy chờ lần đồng bộ tiếp theo." />}</section>
      </div>
