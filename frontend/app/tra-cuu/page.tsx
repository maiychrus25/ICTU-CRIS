...
function SearchContent() {
  ...
  const [filters, setFilters] = useState<WorkFilters>({
    ...
    venue_kind: searchParams.get("venue_kind") || undefined,
  ...
  const showVenueKind = venueOptions.length > 0 && (docType === empty || docType === "bai_bao");
  ...
  const activeFilters = [
    filters.doc_type && docTypeLabels[filters.doc_type],
  ...
  function apply(nextMode = mode, reset = false, syncUrl = true) {
    const next: WorkFilters = reset ? { mode: nextMode, page: 1 } : {
      ...
      venue_kind: (docType === empty || docType === "bai_bao") && venueKind !== empty ? venueKind : undefined,
  ...
  function clearFilters() {
    setQuery(""); setDocType(empty); setYear(empty); setUnit(empty); setVenueKind(empty); setScore(empty); setTopic(empty); setPubType(empty); setQuartile(empty); setCohort(empty);
  ...
  return <>
    ...
    <form onSubmit={(event) => { event.preventDefault(); apply(); }} className="mb-5 rounded-lg border bg-card p-4">
      ...
      <div className="mt-3 hidden gap-3 border-t pt-3 lg:grid lg:grid-cols-3 xl:grid-cols-6">
        <div className="min-w-0"><label className="mb-1 block text-xs font-medium">Loại tài liệu</label><Select value={docType} onValueChange={(value) => setDocType(String(value))}><SelectTrigger aria-label="Loại tài liệu" className="w-full"><SelectValue>{(value) => value === empty ? "Tất cả" : docTypeLabels[String(value)]}</SelectValue></SelectTrigger><SelectContent><SelectItem value={empty}>Tất cả</SelectItem>{Object.entries(docTypeLabels).slice(0, 5).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
        ...
        <FacetSelect label="Chỉ mục" value={pubType} options={facets.data?.pub_types ?? []} onChange={setPubType} />
        {supportsScore && <FacetSelect label="Điểm quy đổi" value={score} options={scoreOptions} onChange={setScore} />}
      </div>
      <details className="mt-3 rounded-md border px-3 py-2 lg:hidden" open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)}><summary className="min-h-9 cursor-pointer content-center text-sm font-medium">Bộ lọc ({activeFilters.length} đang áp dụng)</summary><div className="mt-3 grid min-w-0 gap-3 border-t pt-3 sm:grid-cols-2"><div className="min-w-0"><label className="mb-1 block text-xs font-medium">Loại tài liệu</label><Select value={docType} onValueChange={(value) => setDocType(String(value))}><SelectTrigger aria-label="Loại tài liệu" className="w-full"><SelectValue>{(value) => value === empty ? "Tất cả" : docTypeLabels[String(value)]}</SelectValue></SelectTrigger><SelectContent><SelectItem value={empty}>Tất cả</SelectItem>{Object.entries(docTypeLabels).slice(0, 5).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div><FacetSelect label="Năm" value={year} options={yearOptions} onChange={setYear} /><FacetSelect label="Khoa" value={unit} options={unitOptions} onChange={setUnit} />{showVenueKind && <FacetSelect label="Loại nơi công bố" value={venueKind} options={venueOptions} onChange={setVenueKind} />}<FacetSelect label="Chỉ mục" value={pubType} options={facets.data?.pub_types ?? []} onChange={setPubType} />{supportsScore && <FacetSelect label="Điểm quy đổi" value={score} options={scoreOptions} onChange={setScore} />}<FacetSelect label="Quartile" value={quartile} options={facets.data?.quartiles ?? []} onChange={setQuartile} /><FacetSelect label="Khoá" value={cohort} options={facets.data?.cohorts ?? []} onChange={setCohort} /><div className="min-w-0"><label className="mb-1 block text-xs font-medium">Chủ đề</label><Select value={topic} onValueChange={(value) => setTopic(String(value))}><SelectTrigger aria-label="Chủ đề" className="w-full"><SelectValue>{(value) => value === empty ? "Tất cả" : topics.data?.find((item) => item.id === Number(value))?.label}</SelectValue></SelectTrigger><SelectContent><SelectItem value={empty}>Tất cả</SelectItem>{topics.data?.map((item) => <SelectItem key={item.id} value={String(item.id)}>{item.label} ({item.size})</SelectItem>)}</SelectContent></Select></div></div></details>
      <details className="mt-3 hidden rounded-md border px-3 py-2 lg:block" open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)}><summary className="min-h-8 cursor-pointer content-center text-sm font-medium">Bộ lọc nâng cao</summary><div className="mt-3 grid gap-3 border-t pt-3 sm:grid-cols-3"><FacetSelect label="Quartile" value={quartile} options={facets.data?.quartiles ?? []} onChange={setQuartile} /><FacetSelect label="Khoá" value={cohort} options={facets.data?.cohorts ?? []} onChange={setCohort} /><div className="min-w-0"><label className="mb-1 block text-xs font-medium">Chủ đề</label><Select value={topic} onValueChange={(value) => setTopic(String(value))}><SelectTrigger aria-label="Chủ đề" className="w-full"><SelectValue>{(value) => value === empty ? "Tất cả" : topics.data?.find((item) => item.id === Number(value))?.label}</SelectValue></SelectTrigger><SelectContent><SelectItem value={empty}>Tất cả</SelectItem>{topics.data?.map((item) => <SelectItem key={item.id} value={String(item.id)}>{item.label} ({item.size})</SelectItem>)}</SelectContent></Select></div></div></details>
