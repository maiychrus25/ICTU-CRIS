...
import { useStats, useTopics, useWorks } from "@/lib/queries";
...
function SearchContent() {
  ...
  function submit(event: React.FormEvent) {
    ...
    setFilters({ q: query.trim() || undefined, doc_type: docType === "all" ? undefined : docType, year: year ? Number(year) : undefined, unit: unit === "all" ? undefined : unit, topic: topic === "all" ? undefined : Number(topic), page: 1 });
  }
  ...
  return (
    <>
      ...
      <form onSubmit={submit} className="mb-5 grid gap-3 rounded-lg border bg-card p-4 md:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_150px_110px_150px_180px_auto]">
        <div><label htmlFor="search" className="mb-1.5 block text-xs font-medium">Từ khoá</label><Input id="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tiêu đề (không dấu cũng được) hoặc tên tác giả…" /></div>
        ...
        <div><label className="mb-1.5 block text-xs font-medium">Đơn vị</label><Select value={unit} onValueChange={(value) => setUnit(String(value))} disabled={stats.isLoading}><SelectTrigger aria-label="Đơn vị" className="w-full"><SelectValue>{(value) => value === "all" ? "Tất cả đơn vị" : stats.data?.by_unit.find((item) => item.unit_id === Number(value))?.code}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">Tất cả đơn vị</SelectItem>{stats.data?.by_unit.map((item) => <SelectItem key={item.unit_id} value={String(item.unit_id)}>{item.code} — {item.name}</SelectItem>)}</SelectContent></Select></div>
        <div><label className="mb-1.5 block text-xs font-medium">Chủ đề</label><Select value={topic} onValueChange={(value) => setTopic(String(value))} disabled={topics.isLoading}><SelectTrigger aria-label="Chủ đề" className="w-full"><SelectValue>{(value) => value === "all" ? "Tất cả chủ đề" : topics.data?.find((item) => item.id === Number(value))?.label}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">Tất cả chủ đề</SelectItem>{topics.data?.map((item) => <SelectItem key={item.id} value={String(item.id)}>{item.label} ({item.size})</SelectItem>)}</SelectContent></Select></div>
