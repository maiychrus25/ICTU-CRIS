...
function SearchContent() {
  ...
  const columns = useMemo<DataTableColumn<WorkSummary>[]>(() => [
    ...
    { id: "citation", header: "Trích dẫn", enableSorting: false, cell: ({ row }) => <Button type="button" variant="ghost" size="icon-sm" aria-label={`Trích dẫn ${row.original.title ?? `công trình #${row.original.id}`}`} title="Trích dẫn" onClick={() => setCitationWorkId(row.original.id)}><BookMarked /></Button> },
  ...
  return <>
    ...
    <form onSubmit={(event) => { event.preventDefault(); apply(); }} className="mb-5 rounded-lg border bg-card p-4">
      ...
      <div className="mt-3 grid gap-3 border-t pt-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
