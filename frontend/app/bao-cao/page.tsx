...
function ReportContent() {
  ...
  return <>
    ...
    <section className="mb-7" aria-labelledby="unit-summary-title"><h2 id="unit-summary-title" className="mb-3 text-base font-semibold">Tổng hợp theo khoa</h2>{data.summary.units.length ? <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Khoa</TableHead><TableHead className="text-right">Tổng hồ sơ</TableHead>{stateKeys.map((state) => <TableHead key={state} className={"text-right " + (["DatYeuCau", "DaChot"].includes(state) ? "font-bold" : "")}>{stateLabels[state] ?? state}</TableHead>)}</TableRow></TableHeader><TableBody>{data.summary.units.map((item) => <TableRow key={item.unit_id}><TableCell><span className="font-medium">{item.name}</span><span className="ml-2 text-xs text-muted-foreground">{item.code}</span></TableCell><TableCell className="text-right tabular-nums">{item.declared}</TableCell>{stateKeys.map((state) => <TableCell key={state} className={"text-right tabular-nums " + (["DatYeuCau", "DaChot"].includes(state) ? "font-bold" : "")}>{item.by_state[state] ?? 0}</TableCell>)}</TableRow>)}</TableBody></Table></div> : <EmptyView description="Bản báo cáo không có dữ liệu khoa." />}</section>
    ...
    <section aria-labelledby="report-items-title"><div className="mb-3 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h2 id="report-items-title" className="text-base font-semibold">Chi tiết hồ sơ</h2><p className="text-xs text-muted-foreground tabular-nums">{items.length.toLocaleString("vi-VN")} / {data.items.length.toLocaleString("vi-VN")} hồ sơ</p></div><div className="flex flex-col gap-2 sm:flex-row"><Input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Tìm nhanh hồ sơ" placeholder="Tìm tiêu đề hoặc tác giả…" className="sm:w-72" /><Select value={unit} onValueChange={(value) => setUnit(String(value))}><SelectTrigger aria-label="Lọc khoa" className="sm:w-52"><SelectValue>{(value) => value === "all" ? "Tất cả khoa" : value}</SelectValue></SelectTrigger><SelectContent><SelectItem value="all">Tất cả khoa</SelectItem>{data.summary.units.map((item) => <SelectItem key={item.unit_id} value={item.code}>{item.code} — {item.name}</SelectItem>)}</SelectContent></Select></div></div>{items.length ? <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Hồ sơ</TableHead><TableHead className="min-w-80">Công trình</TableHead><TableHead>Tác giả</TableHead><TableHead>Khoa</TableHead><TableHead>Trạng thái</TableHead><TableHead className="text-right">Minh chứng</TableHead></TableRow></TableHeader><TableBody>{items.map((item) => <TableRow key={item.declaration_id}><TableCell className="font-medium tabular-nums">#{item.declaration_id}</TableCell><TableCell className="whitespace-normal"><span className="font-medium">{item.work.title ?? "Công trình #" + item.work.id}</span><span className="mt-0.5 block text-xs text-muted-foreground">{docTypeLabels[item.work.doc_type] ?? item.work.doc_type} · {item.work.year ?? "Chưa rõ năm"}{item.work.quartile ? " · " + item.work.quartile : ""}{item.work.indexes.length ? " · " + item.work.indexes.join(", ") : ""}</span></TableCell><TableCell className="max-w-xs whitespace-normal">{item.authors.join(", ") || "—"}</TableCell><TableCell>{item.unit.code}</TableCell><TableCell>{stateLabels[item.state] ?? item.state}</TableCell><TableCell className="text-right tabular-nums">{item.evidence_count}</TableCell></TableRow>)}</TableBody></Table></div> : <EmptyView title="Không có hồ sơ phù hợp" description="Đổi từ khoá hoặc bộ lọc khoa để xem hồ sơ khác." />}</section>
  </>;
}
