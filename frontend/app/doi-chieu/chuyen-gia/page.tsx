...
function ExpertResults({ data, units }: { data: ExpertOut; units?: Unit[] }) {
  ...
  const unitLabel = (code: string | null) => units?.find((unit) => unit.code === code) ? `${code} — ${units.find((unit) => unit.code === code)!.name}` : code ?? "Chưa rõ khoa";
