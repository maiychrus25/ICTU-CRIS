...
function FacultyContent() {
  ...
  if (unitId === null) return <><PageHeader title="Góc nhìn khoa" /><EmptyView title="Chưa xác định khoa" description="Tài khoản chưa gắn với khoa và không có quyền chọn khoa khác." /></>;
  ...
  const unitSelector = canChoose ? <Select value={String(unitId)} onValueChange={(value) => router.replace("/khoa/?id=" + value)}><SelectTrigger aria-label="Chọn khoa" className="w-full sm:w-72"><SelectValue>{(value) => unitOptions.find((item) => item.unit_id === Number(value))?.name ?? "Chọn khoa"}</SelectValue></SelectTrigger><SelectContent>{unitOptions.map((item) => <SelectItem key={item.unit_id} value={String(item.unit_id)}>{item.code} — {item.name}</SelectItem>)}</SelectContent></Select> : undefined;
  return <>
    <PageHeader title={selectedUnit?.name ?? data.unit.name} description={openPeriod ? "Số liệu công trình và hồ sơ kê khai trong " + openPeriod.name + "." : "Số liệu công trình và liên kết tác giả của khoa."} action={unitSelector} />
