...
export default function AuthorQueuePage() {
  ...
  return (
    <>
      ...
      {selectedIds.length > 0 && <div className="sticky bottom-4 z-10 mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-popover/95 p-3 shadow-lg backdrop-blur"><p className="text-sm font-medium"><span className="tabular-nums">{selectedIds.length}</span> liên kết đã chọn</p><div className="flex flex-wrap gap-2" title={canDecide ? undefined : officerRoleRequired}><Button type="button" onClick={() => submitDecision({ link_ids: selectedIds, decision: "confirm" })} disabled={!canDecide || decide.isPending}><Check />Xác nhận</Button><Button type="button" variant="destructive" onClick={() => setRejectOpen(true)} disabled={!canDecide}><UserRoundX />Bác bỏ</Button><Button type="button" variant="outline" onClick={() => setReassignOpen(true)} disabled={!canDecide}><CornerDownRight />Chuyển cho người khác</Button></div></div>}
