...
export default function AuthorQueuePage() {
  ...
  async function submitDecision(input: DecideAuthorsIn) {
    try {
      ...
      toast.error(error instanceof ApiError ? error.detail : "Không thể xử lý các liên kết đã chọn.");
    }
  }
  ...
  return (
    <>
      ...
      {selectedIds.length > 0 && <div className="sticky bottom-4 z-10 mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-popover/95 p-3 shadow-lg backdrop-blur"><p className="text-sm font-medium"><span className="tabular-nums">{selectedIds.length}</span> liên kết đã chọn</p><div className="flex flex-wrap gap-2"><Button type="button" onClick={() => submitDecision({ link_ids: selectedIds, decision: "confirm" })} disabled={decide.isPending}><Check />Xác nhận</Button><Button type="button" variant="destructive" onClick={() => setRejectOpen(true)}><UserRoundX />Bác bỏ</Button><Button type="button" variant="outline" onClick={() => setReassignOpen(true)}><CornerDownRight />Chuyển cho người khác</Button></div></div>}

      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent><form onSubmit={(event) => { event.preventDefault(); void submitDecision({ link_ids: selectedIds, decision: "reject", reason: reason.trim() }); }}><DialogHeader><DialogTitle>Bác bỏ liên kết tác giả</DialogTitle><DialogDescription>Nhập lý do để quyết định có thể được kiểm tra lại trong nhật ký.</DialogDescription></DialogHeader><div className="py-4"><label htmlFor="reject-reason" className="mb-1.5 block font-medium">Lý do <span className="text-status-danger">*</span></label><Textarea id="reject-reason" required value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Nêu lý do bác bỏ…" /></div><DialogFooter><Button type="button" variant="outline" onClick={() => setRejectOpen(false)}>Huỷ</Button><Button type="submit" variant="destructive" disabled={decide.isPending}>Bác bỏ</Button></DialogFooter></form></DialogContent>
      </Dialog>
